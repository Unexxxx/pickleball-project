...
begin
  ...
  where attendance.event_id = p_event_id
    and attendance.player_id = p_player_id
    and attendance.state = 'checked_in'
  returning attendance.id into attendance_id;
...
revoke all on function public.organizer_checkout_player(uuid, uuid, uuid) from public;
grant execute on function public.organizer_checkout_player(uuid, uuid, uuid) to authenticated;

comment on function public.organizer_checkout_player(uuid, uuid, uuid) is
  'Checks out an unassigned player and removes their ready entry from the live queue atomically.';
