-- Invitation detail links remain readable after registration closes.
create or replace function public.resolve_event_join(p_join_code text)
returns table(
...
as $$
  ...
  from public.events event join public.clubs club on club.id = event.club_id
  where event.join_code = p_join_code
    and event.status in ('published', 'registration_closed', 'in_progress', 'completed', 'canceled')
