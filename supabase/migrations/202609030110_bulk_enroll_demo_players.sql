...
begin
  ...
  where event.join_code = 'd0ae4f68aba7762143'
    and event.status = 'published';

  if v_event_id is null then
    raise notice 'Demo event was not found; no players were enrolled.';
    return;
  end if;
  ...
  select
    ...
    case
      when v_confirmed_count + candidate.sequence > v_capacity
        then v_waitlist_max + candidate.sequence - greatest(v_capacity - v_confirmed_count, 0)
      else null
    end,
    'admin-bulk-enrollment-2026-09-03'
  ...
  insert into private.audit_log(
    ...
    'event.demo_players_bulk_enrolled',
    'event',
    v_event_id,
    v_club_id,
    ...
    jsonb_build_object(
      ...
      'terms_version', 'admin-bulk-enrollment-2026-09-03'
    )
  );
end
$$;
