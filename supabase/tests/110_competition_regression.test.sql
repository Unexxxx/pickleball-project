...
select ok((select prosrc like '%record_class=''ranked''%' from pg_proc where oid=to_regprocedure('private.finalize_match_result(uuid)')),'only ranked records enter Elo calculation');
...
select ok(
  (select qual not like '%match_participants%' from pg_policies where schemaname='public' and tablename='matches' and policyname='match_participant_or_club_read'),
